...
export class PrevioComponent implements OnInit, AfterViewInit {
  ...
  // serverhostAPIURL = 'http://localhost/apiRits/';
...
<i class="bi bi-pencil-square"></i></button>`,
    ...
    this.tableFracciones = $(tablaFracciones).DataTable({
      ...
      ajax: {
        url: this.serverhostAPIURL + `fracciones`,
        type: 'GET',
        dataType: 'json',
        data(d) {
          d.page = 1;
    ...
    this.tablePartes = $(tablaPartes).DataTable({
      ...
      ajax: {
        url: this.serverhostAPIURL + `partes`,
        type: 'GET',
        dataType: 'json',
        data(d) {
          d.page = 1;
          d.cliente = self.currentCliente || 0;
        }
      },
    });
      ...
      Swal.fire({
        ...
        if (t.isConfirmed) {
          ...
          const options = {
            ...
            body: {
              id: this.currentPartida.id,
              token: '302af6d1e3b16319a274260806b006a7'
            },
          };
          this.http.delete(`${this.serverhostAPIURL}partidas`, options)
            .subscribe(
              response => {
                ...
                if (response.status === 'ok') {
                  self.getPartidas(1, this.dataPrevio.id);
                  this.currentPartida = [];
    ...
    $('#save-change-status').on('click', (e) => {
      ...
      if (newStatus === '') {
        ...
        this.http.put(this.serverhostAPIURL + `clasificaciones`, JSON.stringify(dataClasificacion))
          .subscribe(
            response => {
      ...
      this.servicioValidator.validate().then((status) => {
        if (status === 'Valid') {
          ...
          if (this.currentPartida.id) {
            ...
            this.http.put(this.serverhostAPIURL + `partidas`, JSON.stringify(dataPartida))
              .subscribe(
                response => {
                  ...
                  if (datosTablaIdentificadores.length > 0) {
                    datosTablaIdentificadores.forEach((identificador: { Id: number; Clave: string; Complemento1: string; Complemento2: string; Complemento3: string; }) => {
                      const dataID = {
                        // @ts-ignore
                        id: identificador.Id,
                        clasificacion: dataPartida.IdClasificacion,
                        clave: identificador.Clave,
                        c1: identificador.Complemento1,
                        c2: identificador.Complemento2,
                        c3: identificador.Complemento3,
                        token: '302af6d1e3b16319a274260806b006a7'
                      };
                      this.http.post(this.serverhostAPIURL + `identificadores`, JSON.stringify(dataID))
                        .subscribe(
                          response => {
                  ...
                  if (datosTablaPermisos.length > 0) {
                    datosTablaPermisos.forEach((permiso: { Id: number; IdPedimento: string; Clave: string; FirmaDescargo: string; NumeroPermiso: string; ValorComercialDolares: string; CantidadMercancia: string; }) => {
                      ...
                      this.http.post(this.serverhostAPIURL + `permisos`, JSON.stringify(dataID))
                        .subscribe(
                          response => {
                            console.log('RESPONSE ID', response);
                          },
            ...
            this.http.post(this.serverhostAPIURL + `partidas`, JSON.stringify(dataPartida))
              .subscribe(
                response => {
                  if (datosTablaIdentificadores.length > 0) {
                    datosTablaIdentificadores.forEach((identificador: { Id: number; Clave: string; Complemento1: string; Complemento2: string; Complemento3: string; }) => {
          ...
          const dataClasificacion = {
            id: this.dataPrevio.id,
            estado: this.dataPrevio.estatusId,
            token: '302af6d1e3b16319a274260806b006a7'
          };
          Swal.fire({
            title: 'Qué estado quiere asignarle al previo?',
            icon: 'info',
            showDenyButton: true,
            showCancelButton: true,
            confirmButtonText: 'Finalizar',
            denyButtonText: `En pausa`,
            cancelButtonText: `Clasificando/Cerrar`,
            buttonsStyling: !1,
            customClass: {confirmButton: 'btn btn-success', denyButton: 'btn btn-primary', cancelButton: 'btn btn-active-light'}
          }).then((result) => {
            if (result.isConfirmed) {
              dataClasificacion.estado = 3351;
            } else if (result.isDenied) {
              dataClasificacion.estado = 3349;
            } else {
              dataClasificacion.estado = 3348;
            }
  ...
  getRevision(previo) {
    fetch(this.serverhostAPIURL + `clasificaciones?id=${previo}`)
      .then(response => response.json())
      .then(data => {
        this.dataPrevio = data[0];
        // tslint:disable-next-line:radix
        this.currentCliente = parseInt(this.dataPrevio.clienteId);
        this.getPartidas(1, this.dataPrevio.id);
...
console.log('previo', this.dataPrevio)
        $('#estado-previo-lbl').text(this.dataPrevio.estatusDescripcion);
        this.tablePartes.ajax.reload();
      })
      .catch(error => console.error('Error en la solicitud:', error));
  }
  getPartidas(page, clasificacion) {
    fetch(this.serverhostAPIURL + `partidas?page=${page}&clasificacion=${clasificacion}`)
      .then(response => response.json())
      .then(data => {
  ...
  getIDPartidas(clasificacion) {
    this.tableIdentificadores.clear().draw();
    fetch(this.serverhostAPIURL + `identificadores?id=${clasificacion}`)
      .then(response => response.json())
      .then(data => {
  ...
  getPermisosPartidas(clasificacion) {
    this.tablePermisos.clear().draw();
    fetch(this.serverhostAPIURL + `permisos?id=${clasificacion}`)
      .then(response => response.json())
      .then(data => {
  ...
  getCatalogos() {
    fetch(this.serverhostAPIURL + `catalogos?page=1&umc`)
      .then(response => response.json())
      .then(data => {
        this.listaUmc = data;
      })
      .catch(error => console.error('Error en la solicitud:', error));
    fetch(this.serverhostAPIURL + `catalogos?paises`)
      .then(response => response.json())
      .then(data => {
        this.listaPaises = data;
      })
      .catch(error => console.error('Error en la solicitud:', error));
    fetch(this.serverhostAPIURL + `catalogos?zonas`)
      .then(response => response.json())
      .then(data => {
        this.listaZonas = data;
      })
      .catch(error => console.error('Error en la solicitud:', error));
    fetch(this.serverhostAPIURL + `catalogos?previostatus`)
      .then(response => response.json())
      .then(data => {
  ...
  openDrawerPartida(partida: string, index?: number) {
    ...
    // @ts-ignore
    const drawer = KTDrawer.getInstance(drawerElement);
    fetch(this.serverhostAPIURL + `partidas?id=${partida}`)
      .then(response => response.json())
      .then(data => {

        this.currentPartida = data[0];
...
<a class="d-block overlay mb-3 w-200px" data-lightbox="lightbox-basic" href="${imagen.Ruta}">
    ...
    <div class="overlay-layer card-rounded bg-dark bg-opacity-25 shadow">
        ...
        this.getIDPartidas(this.currentPartida.clasificacion_id);
        this.getPermisosPartidas(this.currentPartida.clasificacion_id);
  ...
  eliminarIdentificador(identificador) {
    ...
    this.http.delete(`${this.serverhostAPIURL}identificadores`, options)
      .subscribe(
        response => {
  ...
  eliminarPermiso(permiso) {
    ...
    this.http.delete(`${this.serverhostAPIURL}permisos`, options)
      .subscribe(
        response => {
